...
class UploadAttachments
	def self.upload_attachments
		...
		request = ZOHOCRMSDK::Attachments::FileBodyWrapper.new
		sw = ZOHOCRMSDK::Util::StreamWrapper.new(nil, nil, file_path)
		request.file = sw
		
		response = attachments_operations.upload_attachments(record_id, module_name, request)
		if response != nil
					...
					if action_responses != nil
						action_responses.each_with_index do |action_response, index|
							if action_response.is_a? ZOHOCRMSDK::Attachments::SuccessResponse
								...
								print "Attachment #{index + 1} - Upload Success:\n"
								...
								print "Attachment #{index + 1} - Upload Error:\n"
