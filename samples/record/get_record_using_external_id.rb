...
class GetRecordUsingExternalId
	def self.get_record_using_external_id
		begin
			# Initialize RecordOperations with module name
			record_operations = ZOHOCRMSDK::Record::RecordOperations.new("Leads")
			
			# Specify the external field value (e.g., external ID, email, etc.)
			external_field_value = "External2"
			
			# Create parameter map for optional parameters
			param_instance = ZOHOCRMSDK::ParameterMap.new
			
			# Add optional parameters
			# param_instance.add(ZOHOCRMSDK::Record::RecordOperations::GetRecordUsingExternalIDParam.fields, "Last_Name,First_Name,Company,Email,Phone,External_ID")
			param_instance.add(ZOHOCRMSDK::Record::RecordOperations::GetRecordUsingExternalIDParam.approved, "both")
			param_instance.add(ZOHOCRMSDK::Record::RecordOperations::GetRecordUsingExternalIDParam.converted, "both")
			...
			header_instance.add(ZOHOCRMSDK::Record::RecordOperations::GetRecordUsingExternalIDHeader.X_external, "Leads.External")
			...
			if response != nil
				...
				if [204, 304].include?(response.status_code)
					print (response.status_code == 204 ? "No Content - Record not found" : "Not Modified") + "\n"
					return
				end
...
GetRecordUsingExternalId.initialize_and_call