...
class RemoveTerritoriesFromMultipleRecords
	def self.remove_territories_from_multiple_records
		begin
			...
			record.id = 1055806000029102002  # Replace with actual territory ID
			
			territory = ZOHOCRMSDK::Record::Territory.new
			territory.id = 1055806000003051397  # Replace with actual territory ID
			record.add_key_value("Territories", [territory])  # Replace "" with actual field API name for territory in the module
					...
					if response_handler.is_a? ZOHOCRMSDK::Record::ActionWrapper
						...
						action_response_list.each_with_index do |action_response, index|
							if action_response.is_a? ZOHOCRMSDK::Record::SuccessResponse
								...
								print "Territory Removal Operation #{index + 1} Status: SUCCESS\n"
								...
								print "Territory Removal Operation #{index + 1} Status: ERROR\n"
						...
						print "Remove Territories from Multiple Records API Exception:\n"
	...
	def self.initialize_and_call
		begin
			...
			remove_territories_from_multiple_records
		rescue Exception => e
			raise e
		end
	end
...
RemoveTerritoriesFromMultipleRecords.initialize_and_call