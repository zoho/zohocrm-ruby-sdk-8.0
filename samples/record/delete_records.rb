...
class DeleteRecords
	def self.delete_records
		begin
			...
			# Create parameter map with record IDs to delete
			param_instance = ZOHOCRMSDK::ParameterMap.new
			
			# Add record IDs to delete (replace with actual record IDs)
			record_ids = [1055806000028884054, 1055806000028884053, 347706116859015]
			param_instance.add(ZOHOCRMSDK::Record::RecordOperations::DeleteRecordsParam.ids, record_ids.join(","))
			
			# Add optional parameter to trigger workflow
			param_instance.add(ZOHOCRMSDK::Record::RecordOperations::DeleteRecordsParam.wf_trigger, true)
...
DeleteRecords.initialize_and_call