...
class RemoveTags
	def self.remove_tags
		begin
			...
			# Create request body
			existing_tag_request_wrapper = ZOHOCRMSDK::Tags::ExistingTagRequestWrapper.new
			tags = []

			tag = ZOHOCRMSDK::Tags::ExistingTag.new
			tag.name = "New Record Tag"
			tags.push(tag)

			existing_tag_request_wrapper.tags = tags
			
			response = tags_operations.remove_tags(module_api_name, record_id, existing_tag_request_wrapper)
...
RemoveTags.initialize_and_call