...
class DeleteVariables
	def self.delete_variables
		begin
			# Initialize VariablesOperations
			variables_operations = ZOHOCRMSDK::Variables::VariablesOperations.new
			
			# Create ParameterMap for variable IDs to delete
			param_instance = ZOHOCRMSDK::ParameterMap.new
			
			# Add multiple variable IDs to delete
			variable_ids = ["1055806000011121011", "1055806000010321012"]  # Replace with actual IDs
			variable_ids.each do |variable_id|
				param_instance.add(ZOHOCRMSDK::Variables::VariablesOperations::DeleteVariablesParam.ids, variable_id)
			end
...
DeleteVariables.initialize_and_call