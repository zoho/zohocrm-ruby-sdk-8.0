...
class GetCurrency
	def self.get_currency
		...
		currency_id = 1055806000005657003
			...
			if response.is_expected
				...
				if response_handler.is_a? ZOHOCRMSDK::Currencies::ResponseWrapper
					...
					if currencies_list != nil && currencies_list.length > 0
						...
						# Additional currency properties
						if currency.prefix_symbol != nil
							print "Prefix Symbol: " + currency.prefix_symbol.to_s + "\n"
						end
					else
						print "Currency with ID " + currency_id.to_s + " not found.\n"
					end
...
GetCurrency.initialize_and_call