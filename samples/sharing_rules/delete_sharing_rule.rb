...
class DeleteSharingRule
		def self.delete_sharing_rule(id, module_api_name)
			sharing_rules_operations =  ZOHOCRMSDK::SharingRules::SharingRulesOperations.new(module_api_name)
			response = sharing_rules_operations.delete_sharing_rule(id)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 3477059001
				delete_sharing_rule(id, module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
DeleteSharingRule.initialize_and_call