...
class DeactivateSharingRule
		def self.deactivate_sharing_rule(id, module_api_name)
			sharing_rules_operations =  ZOHOCRMSDK::SharingRules::SharingRulesOperations.new(module_api_name)
			response = sharing_rules_operations.deactivate_sharing_rule(id)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 34771001
				deactivate_sharing_rule(id, module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
DeactivateSharingRule.initialize_and_call