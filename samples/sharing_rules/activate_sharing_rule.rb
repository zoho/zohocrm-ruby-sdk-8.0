...
class ActivateSharingRule
		def self.activate_sharing_rule(id, module_api_name)
			sharing_rules_operations = ZOHOCRMSDK::SharingRules::SharingRulesOperations.new(module_api_name)
			response = sharing_rules_operations.activate_sharing_rule(id)
			if response != nil
	...
	def self.initialize_and_call
			begin
				...
				id = 347701001
				activate_sharing_rule(id, module_api_name)
			rescue Exception => e
				raise e
			end
		end
end
ActivateSharingRule.initialize_and_call
