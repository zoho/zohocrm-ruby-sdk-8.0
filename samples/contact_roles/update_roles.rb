...
class UpdateRoles
	def self.update_roles
		...
		# Create list of contact roles to update
		contact_roles_list = []
		
		# Update first contact role
		contact_role1 = ZOHOCRMSDK::ContactRoles::ContactRole.new
		contact_role1.id = 1055806000016838002  # Replace with actual role ID
		contact_role1.name = "Chief Decision Maker"
		contact_role1.sequence_number = 1
		contact_roles_list.push(contact_role1)
		
		# Update second contact role
		contact_role2 = ZOHOCRMSDK::ContactRoles::ContactRole.new
		contact_role2.id = 1055806000000691006  # Replace with actual role ID
		contact_role2.name = "Senior Technical Evaluator"
...
UpdateRoles.initialize_and_call