...
class GetVariableGroupByApiName
	def self.get_variable_group_by_api_name(api_name)
		begin
				...
				if response.is_expected
					...
					if response_handler.is_a? ZOHOCRMSDK::VariableGroups::ResponseWrapper
						...
						if variable_groups != nil && variable_groups.length > 0
							variable_groups.each_with_index do |variable_group, index|
	...
	def self.initialize_and_call
		begin
			...
			# Replace with actual variable group API name
			api_name = "General"
...
GetVariableGroupByApiName.initialize_and_call