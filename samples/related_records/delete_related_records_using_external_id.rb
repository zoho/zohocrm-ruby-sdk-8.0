...
class DeleteRelatedRecordsUsingExternalId
	def self.delete_related_records_using_external_id
		begin
			...
			# Create list of external IDs for bulk deletion
			external_ids = ["Products_External", "external_related_id_2", "external_related_id_3"] # Replace with actual external IDs
			
			# Create parameter map for external IDs
			param_instance = ZOHOCRMSDK::ParameterMap.new
			param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::DeleteRelatedRecordsUsingExternalIDParam.ids, external_ids.join(","))
			
			# Create header map for optional headers
			header_instance = ZOHOCRMSDK::HeaderMap.new
			header_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::DeleteRelatedRecordsUsingExternalIDHeader.X_external, "Leads.External,Products.Products_External")
			
			response = related_records_operations.delete_related_records_using_external_id(external_id, param_instance, header_instance)
	...
	def self.initialize_and_call
		begin
			...
			delete_related_records_using_external_id
		rescue Exception => e
			raise e
		end
	end
