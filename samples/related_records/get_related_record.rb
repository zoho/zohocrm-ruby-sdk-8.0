...
class GetRelatedRecord
	def self.get_related_record
		begin
			...
			# param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordParam.fields, "Note_Title,Note_Content,Created_Time,Modified_Time")
			
			# Create header map for optional headers
			header_instance = ZOHOCRMSDK::HeaderMap.new
			# header_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordHeader.If_modified_since, DateTime.new(2023, 1, 1, 10, 0, 0))
			
			response = related_records_operations.get_related_record(related_record_id, record_id, param_instance, header_instance)
			
			if response != nil
				print "Status Code: " + response.status_code.to_s + "\n"
				
				if [204, 304].include?(response.status_code)
					print (response.status_code == 204 ? "No Content - Related record not found" : "Not Modified") + "\n"
					return
				end
...
GetRelatedRecord.initialize_and_call