...
class UpdateRelatedRecordsUsingExternalId
	def self.update_related_records_using_external_id
		begin
			...
			# Create records list for update
			record = ZOHOCRMSDK::Record::Record.new
			record.add_key_value("Products_External", "Products_External")
			...
			header_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::UpdateRelatedRecordsUsingExternalIDHeader.X_external, "Leads.External,Products.Products_External")
			response = related_records_operations.update_related_records_using_external_id(external_id, request, header_instance)
	...
	def self.initialize_and_call
		begin
			...
			update_related_records_using_external_id
		rescue Exception => e
			raise e
		end
	end
...
UpdateRelatedRecordsUsingExternalId.initialize_and_call