...
class DelinkRecord
	def self.delink_record
		begin
			...
			response = related_records_operations.delink_record(related_record_id, record_id, header_instance)
			
			if response != nil
				print "Status Code: " + response.status_code.to_s + "\n"
				
				if [204, 304].include?(response.status_code)
					print (response.status_code == 204 ? "No Content - Record delinked successfully" : "Not Modified") + "\n"
					return
				end
...
DelinkRecord.initialize_and_call