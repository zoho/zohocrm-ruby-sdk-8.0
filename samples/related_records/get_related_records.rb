...
class GetRelatedRecords
	def self.get_related_records
		begin
			# Set the record ID and related list API name
			record_id = 1055806000028898011 # Replace with actual record ID
			module_api_name = "Leads"
			related_list_api_name = "Products" # Related list API name (e.g., Notes, Attachments, etc.)
			...
			# param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordsParam.fields, "Product_Name,Product_Code,Created_Time")
			param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordsParam.page, 1)
			param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordsParam.per_page, 200)
			param_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordsParam.ids, "1055806000006611017,1055806000006611017")
			
			# Create header map for optional headers
			header_instance = ZOHOCRMSDK::HeaderMap.new
			# header_instance.add(ZOHOCRMSDK::RelatedRecords::RelatedRecordsOperations::GetRelatedRecordsHeader.If_modified_since, DateTime.new(2023, 1, 1, 10, 0, 0))
