...
class GetAssociatedContactRolesSpecificToContact
	def self.get_associated_contact_roles_specific_to_contact
		...
		response = deal_contact_roles_operations.get_associated_contact_roles_specific_to_contact(contact_id, deal_id)
		if response != nil
			print "Status Code: " + response.status_code.to_s + "\n"
			
			if response.status_code == 204
				print "No Content - No contact role association found\n"
				print "Contact ID #{contact_id} is not associated with Deal ID #{deal_id}\n"
				return
			end
	...
	def self.initialize_and_call
		begin
			...
			get_associated_contact_roles_specific_to_contact
		rescue Exception => e
			raise e
		end
	end
...
GetAssociatedContactRolesSpecificToContact.initialize_and_call