...
class GetTax
	def self.get_tax
		begin
			# Set tax ID
			id = 1055806000013694003  # Replace with actual tax ID
			
			# Initialize TaxesOperations
			taxes_operations = ZOHOCRMSDK::Taxes::TaxesOperations.new
			
			response = taxes_operations.get_tax(id)
			
			if response != nil
				print "Status Code: " + response.status_code.to_s + "\n"
				
				if [204, 304].include?(response.status_code)
					print (response.status_code == 204 ? "No Content - Tax not found" : "Not Modified") + "\n"
					return
				end
