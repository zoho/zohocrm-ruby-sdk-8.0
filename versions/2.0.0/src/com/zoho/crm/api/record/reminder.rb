  ...
  module Record
    class Reminder
      ...
      def unit
        ...
        # @param unit [Integer] A Integer

      def unit=(unit)
        if unit!=nil and !unit.is_a? Integer
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: unit EXPECTED TYPE: Integer', nil, nil)
        end
        @unit = unit
        @key_modified['unit'] = 1
      end
      ...
      def time
        ...
        # @param time [String] A String

      def time=(time)
        if time!=nil and !time.is_a? String
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: time EXPECTED TYPE: String', nil, nil)
        end
        @time = time
        @key_modified['time'] = 1
      end
