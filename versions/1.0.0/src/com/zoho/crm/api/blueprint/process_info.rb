  ...
  module Blueprint
    class ProcessInfo
      ...
      def field_id=(field_id)
        if field_id!=nil and !field_id.is_a? Integer
      ...
      def escalation=(escalation)
        ...
        # @return An instance of CurrentPicklist

      def current_picklist
        @current_picklist
      end

        # The method to set the value to current_picklist
        # @param current_picklist [CurrentPicklist] An instance of CurrentPicklist

      def current_picklist=(current_picklist)
        if current_picklist!=nil and !current_picklist.is_a? CurrentPicklist
          raise SDKException.new(Constants::DATA_TYPE_ERROR, 'KEY: current_picklist EXPECTED TYPE: CurrentPicklist', nil, nil)
        end
        @current_picklist = current_picklist
        @key_modified['current_picklist'] = 1
      end
