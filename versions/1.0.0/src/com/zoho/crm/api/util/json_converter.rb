    ...
    class JSONConverter < Converter
      ...
      def is_record_request(record_instance, class_detail, instance_number, class_member_details = nil)
        ...
        unless skip_mandatory
          module_detail.each_key do |key_name|
            ...
            if !key_detail.nil? && key_detail.key?(Constants::NAME)
              name = key_detail[Constants::NAME]

              if key_detail.key?(Constants::REQUIRED) && key_detail[Constants::REQUIRED] == true
                required_keys[name] = 1
              end

              if key_detail.key?(Constants::PRIMARY) && key_detail[Constants::PRIMARY] == true
                primary_keys[name] = 1
              end
            end
          end
        ...
        key_modified.each_key do |key_name|
          ...
          if !key_value.nil?
            if !key_detail.nil? && !key_detail.empty?
              ...
              if custom_handling && !key_value.is_a?(Array) && !key_value.is_a?(Hash) && !key_value.is_a?(Choice)
                key_value_class_name = Util::Utility.path_to_package(key_value.class.name)
                if Constants::BOOLEAN_CLASSES.include?(key_value_class_name)
                  key_detail[Constants::TYPE] = Constants::BOOLEAN
                  json_value = set_data(key_detail, key_value)
                elsif Constants::PRIMITIVE_TYPES.include?(key_value_class_name)
                  key_detail[Constants::TYPE] = key_value.class.name
                  json_value = set_data(key_detail, key_value)
                elsif Initializer.json_details.include?(key_value_class_name)
                  key_detail[Constants::STRUCTURE_NAME] = key_value_class_name
                  key_detail[Constants::NAME] = key_name
                  key_detail[Constants::TYPE] = key_value_class_name
                  json_value = set_data(key_detail, key_value)
                end
              else
                if custom_handling && key_value.is_a?(Array)
                  key_value_class_name = Util::Utility.path_to_package(key_value[0].class.name)
                  if Initializer.json_details.include?(key_value_class_name)
                    key_detail[Constants::STRUCTURE_NAME] = key_value_class_name
                    key_detail[Constants::NAME] = key_name
                    key_detail[Constants::TYPE] = key_value_class_name
                    json_value = set_json_array(key_value, key_detail)
                  else
      ...
      def get_wrapped_response(response, pack)
        if !response.body.nil? && !response.body.length.zero?
          ...
          [get_response(response_json, pack), response_json]
        end
      rescue StandardError => e
        raise e
      end
      ...
      def get_response(response_json, pack)
        ...
        unless pack.nil?
          ...
          if pack.to_s.include? Constants::DOUBLE_COLON
            ...
            module_name = Utility.classname_to_filename(class_path_split[1])
            file_name = Utility.classname_to_filename(class_path_split[2])
          else
        ...
        unless response.empty?
          ...
          if member_detail.nil? || (!member_detail.nil? && !member_detail.include?(Constants::KEYS))
            ...
            keys_detail.each do |key_detail|
              ...
              if !response.is_a?(Array) && response.key?(key_name) && !response[key_name].nil?
      ...
      def get_module_detail_from_user_spec_json(module_name)
        record_field_details_path = Initializer.get_initializer.resources_path + File::Separator + Constants::FIELD_DETAILS_DIRECTORY + File::Separator + Converter.new(nil).get_record_json_file_path
        record_field_details = JSON.parse(File.read(record_field_details_path))
        module_detail = Utility.get_json_object(record_field_details, module_name.downcase)
        if !module_detail.nil? && module_detail.length == 0
          File.open(record_field_details_path, 'w') do |file|
            record_field_details.delete[module_name.downcase]
            file.write(JSON.generate(record_field_details))
          end
        end
        module_detail
      end
        ...
        class_json_details.each_key do |member_name|
          ...
          if !key_name.nil? && response_json.key?(key_name.to_s) && !response_json[key_name].nil?
            ...
            if type == member_json_details[Constants::TYPE]
              matches += 1
            elsif key_name.downcase == Constants::COUNT && type.downcase == Constants::INTEGER.downcase
              matches += 1
